...
class AuthController {
    ...
    async registrar(usuarioData) {
        try {
            ...
            const [emailExiste] = await db.query('SELECT email FROM usuarios WHERE email = ?', [usuarioData.email]);
            ...
            const saltRounds = 10;
            const hashedPassword = await bcrypt.hash(usuarioData.clave, saltRounds);
